  ...
  return (
    <>
      <div className="fixed bottom-0 left-0 right-0 bg-background/95 backdrop-blur-sm border-t border-border z-50">
        <div className="flex items-center gap-4 px-4 py-3 max-w-6xl mx-auto relative">
          {/* Overlay for Now Playing click (except controls) */}
          <div
            className="absolute inset-0 z-10 cursor-pointer"
            onClick={e => {
              // Prevent click if controls are clicked
          ...
          {/* Controls */}
          <div className="flex items-center gap-3 player-controls z-20">
            {/* Skip Back 10s */}
            <Button
              variant="ghost"
              size="sm"
      ...
      <NowPlayingModal open={showNowPlaying} onClose={() => setShowNowPlaying(false)} />
    </>
  );
};
